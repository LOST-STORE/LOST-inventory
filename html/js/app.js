            ...
            if (ItemData.name.split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    $(".inv-options-list").append(
                        ...
                        '<div style="height: 100%;background-color:#444554;margin-top:29.4vh" class="inv-option-item" id="weapon-attachments"><p>ATTACHMENTS</p></div>'
                    );
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://LOST-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://LOST-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://LOST-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://LOST-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://LOST-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://LOST-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://LOST-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://LOST-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://LOST-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("https://LOST-inventory/CloseInventory", JSON.stringify({}));
    ...
    Inventory.Clickables = function () {
        ...
        $('.cloth').find('img').click(function () {
            $.post("http://LOST-inventory/ChangeComponent", JSON.stringify({
    ...
    Inventory.Necessary = function () {
        ...
        $('#clothing-menu').click(function () {
            if (isClothOpen == false) {
                isClothOpen = true
                $.post("http://LOST-inventory/OpenClothMenu", JSON.stringify({ delete: isClothOpen }));
    ...
    window.onload = function(e) {
        window.addEventListener("message", function(event) {
            switch (event.data.action) {
                ...
                case "RobMoney":
                    $(".inv-options-list").append(
                        '<center><div class="inv-option-rob" id="rob-money"><p>سرقة الكاش</p></div></center>'
                    );
